...
if [ $1 -gt $2 ];then
    ...
    if [ $? -eq 1 ];then
        ...
        save_log "info" "邻居mia的$3己下调优化先为与其它线路动态调整,LP值动态变化"
    fi
else
    save_log "info" "邻居$3丢包在可控范围内"
    if_run_bebore "$3_restore.sh" "last_run_$3"
    if [ $? -eq 1 ];then
        sh $script_dir/${3}_restore.sh
        echo "${3}_restore.sh" > $path_dir/last_run_$3
        save_log "info" "邻居mia的$3己恢复优先级LP:600"
    fi
...
compare_rate "$mia_avg" "$loss_rate" "facebook_routes"
}
main $*